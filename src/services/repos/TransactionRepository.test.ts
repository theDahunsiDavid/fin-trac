...
describe('TransactionRepository', () => {
  ...
  describe('getTransactionsByDateRange', () => {
    it('returns transactions within date range', async () => {
      ...
      const result = await repository.getTransactionsByDateRange(startDate, endDate);

      expect(result).toEqual(mockTransactions);
      expect(mockTransactionsTable.where).toHaveBeenCalledWith('date');
      expect(mockTransactionsTable.between).toHaveBeenCalledWith(startDate, endDate, true, true);
    });
  });
