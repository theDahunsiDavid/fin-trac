 ...
 *
 * Manages configuration for CouchDB synchronization including environment variables,
...
export class SyncConfigService {
  ...
  validateConfig(): { valid: boolean; errors: string[] } {
    ...
    if (!this.config.database || !/^[a-z][a-z0-9_$()+/-]*$/.test(this.config.database)) {
      errors.push('Invalid database name (must be lowercase, start with letter, contain only a-z, 0-9, _, $, (, ), +, /, -)');
    }

    // Validate credentials (both or neither)
    if ((this.config.username && !this.config.password) || (!this.config.username && this.config.password)) {
      errors.push('Both username and password must be provided for authentication');
    }

    // Validate numeric values
    if (this.config.syncInterval && (this.config.syncInterval < 1000 || this.config.syncInterval > 3600000)) {
